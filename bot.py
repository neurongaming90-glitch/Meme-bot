...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Step 1: Big welcome message
    name = user.first_name or "Bhai"
    line = "\u2501" * 23
    welcome_text = (
        "<b>" + "🎭 InstasMeme Bot mein Welcome " + name + "!</b> 😈\n\n" +
        line + "\n" +
        "🤖 <b>Main kya hun?</b>\n" +
        line + "\n" +
        "Main tera savage meme dost hun! 😏\n" +
        "Jo bhi meme chahiye — bas bol, main dhundh ke dunga.\n\n" +
        line + "\n" +
        "⚡ <b>Main kya kar sakta hun:</b>\n" +
        line + "\n" +
        "📷 Photo Memes — Funny, dark, dank sab\n" +
        "🎬 Video Memes — YouTube se best clips\n" +
        "🔊 Sound Memes — Viral audio effects\n" +
        "💬 AI Chat — Roast + normal baat\n\n" +
        line + "\n" +
        "🎯 <b>Example:</b>\n" +
        "<i>Modi ji meme chahiye</i>\n" +
        "<i>Funny cat video do</i>\n" +
        "<i>IPL meme sound chahiye</i>\n\n" +
        line + "\n" +
        "🔥 Sources: YouTube + Instagram\n" +
        line + "\n" +
        "👇 <b>Help dabao ya seedha meme maango!</b>"
    )

    keyboard = [
        [InlineKeyboardButton("👑 Owner @ethicalrobo", url=f"https://t.me/{OWNER_USERNAME.lstrip('@')}")],
    ...
    await update.message.reply_text(
        ...
        # Step 2: AI savage intro after welcome
    await asyncio.sleep(1)
    intro_text = get_intro(name, lang)
...
async def verify_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_member(context.bot, uid):
        ...
        line = "\u2501" * 23
        welcome_text = (
            "<b>✅ Verified! Welcome " + name + "!</b> 🎉\n\n" +
            "🎭 <b>InstasMeme Bot Ready hai!</b> 😈\n\n" +
            line + "\n" +
            "⚡ <b>Main kya kar sakta hun:</b>\n" +
            line + "\n" +
            "📷 Photo Memes — Funny, dark, dank\n" +
            "🎬 Video Memes — YouTube se best clips\n" +
            "🔊 Sound Memes — Viral audio effects\n" +
            "💬 AI Chat — Roast + normal baat\n\n" +
            line + "\n" +
            "🎯 <b>Example:</b> <i>Modi ji meme do</i>\n" +
            line + "\n" +
            "👇 <b>Seedha meme maango!</b>"
        )
        keyboard = [
            [InlineKeyboardButton("👑 Owner @ethicalrobo", url=f"https://t.me/{OWNER_USERNAME.lstrip('@')}")],
            [InlineKeyboardButton("❓ Help and Commands", callback_data="show_help")]
...
async def ask_format(message, query_text: str, style: str, uid: int):
    ...
    keyboard = [
        [
            InlineKeyboardButton("📷 Photo Template", callback_data="format_photo"),
            InlineKeyboardButton("🎬 Video/Short", callback_data="format_video"),
        ],
        [
            InlineKeyboardButton("🔊 Meme Sound", callback_data="format_sound"),
            InlineKeyboardButton("🔥 Trending", callback_data="format_trending"),
        ]
    ]
    lang = get_state(uid).get('lang', 'hinglish')
    texts = {
...
async def format_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Trending — special flow, no query needed
    if fmt == 'trending':
        await query.edit_message_text("🔥 Trending meme templates dhundh raha hun...")
        await send_memes(query.message, uid, context)
        return

    emoji = '📷' if fmt == 'photo' else '🎬' if fmt == 'video' else '🔊'
    loading_texts = {
        'hindi': f"{emoji} Dhundh raha hun...",
        'english': f"{emoji} Searching...",
        'hinglish': f"{emoji} Dhundh raha hun bhai..."
    }
...
async def send_memes(message, uid: int, context):
    ...
    # Fetch memes
    if fmt == 'trending':
        result = fetch_memes(query_text, media_type='trending', count=6)
    else:
    ...
    for i, meme in enumerate(memes[:MEMES_PER_BATCH]):
        try:
            ...
            elif meme_type == 'video_file':
                # Downloaded file — send directly
                import os
                file_path = meme['url']
                ...
                if os.path.exists(file_path):
                    with open(file_path, 'rb') as af:
                        await context.bot.send_audio(
                            ...
                            caption=meme_caption,
                            parse_mode="HTML",
                            title=meme.get('title', 'Meme Sound')[:60]
                        )
                    try:
                        os.remove(file_path)
                    except Exception:
            ...
            elif meme_type == 'audio' or fmt == 'sound':
                # URL-based audio
                try:
                    await context.bot.send_audio(
                        uid, meme['url'],
            ...
            elif meme_type == 'video' or fmt == 'video':
                # Stream URL — may fail, fallback to thumbnail
                try:
                    await context.bot.send_video(
                        uid, meme['url'],
                        caption=meme_caption,
                    ...
                    thumb = meme.get('thumbnail', '')
                    vid_link = meme.get('video_url', '')
                    if thumb and vid_link:
                        cap2 = (meme_caption or '') + f"\n\n▶️ <a href='{vid_link}'>Watch on YouTube</a>"
                        await context.bot.send_photo(uid, thumb, caption=cap2, parse_mode="HTML")
                    else:
                        raise
            ...
            logger.warning(f"Failed to send meme {i+1}: {e}")
            continue
